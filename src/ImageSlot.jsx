...
export default function ImageSlot({ id, placeholder = "Drop image", radius = 12, readOnly = false }) {
  ...
  if (src) {
    return (
      <div
        style={{ width: "100%", height: "100%", borderRadius: radius, overflow: "hidden", cursor: readOnly ? "default" : "pointer", position: "relative" }}
        ...
        onDrop={readOnly ? undefined : (e) => { e.preventDefault(); setOver(false); handleFile(e.dataTransfer.files[0]); }}
      >
        <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        {!readOnly && <input ref={inputRef} type="file" accept="image/*" hidden onChange={(e) => handleFile(e.target.files[0])} />}
      </div>
    );
  }
  ...
  if (readOnly) {
    return (
      <div style={{
        ...
        borderRadius: radius, background: "#1A2028", color: "#9CA3AF", fontSize: 13,
      }}>
